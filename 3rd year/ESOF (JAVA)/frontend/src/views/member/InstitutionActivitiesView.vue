...
export default class InstitutionActivitiesView extends Vue {
  ...
  async manageShifts(activity: Activity) {
    await this.$store.dispatch('setActivity', activity);
    await this.$router.push({ name: 'institution-activity-shifts', params: { activityId: String(activity.id) } });
  }

  getShiftsCapacity(activity: Activity): string {
    const v = activity.shifts
      ? activity.shifts.reduce((sum, shift) => sum + shift.participantsLimit, 0)
      : 0;

    const t = activity.participantsNumberLimit;
